...
class Dashboard extends React.Component<any, any> {
  ...
  componentDidUpdate() {
    if (
      ...
      if (
        `${this.props.match.path}/home` !== '/dashboard/home' &&
      ...
      this.props.auth.currentUser.roles.some(
        (role) => role === cognitoRoles.TRAINER,
      )
    ) {
  ...
  render() {
    ...
    return (
      <div id="manage-users-container">
        <Switch>
          <ProtectedRoute
            allowedRoles={['admin', 'staging-manager']}
            path={`${path}/home`}
            component={Home}
          />
          <ProtectedRoute
          ...
          <ProtectedRoute
            allowedRoles={['admin', 'staging-manager']}
            path={`${path}/fiveOrMore`}
            component={FiveOrMore}
          />
          <ProtectedRoute
            allowedRoles={['admin', 'staging-manager']}
            path={`${path}/numInterviews`}
            component={NumInterviews}
          />
          <ProtectedRoute
            allowedRoles={['admin', 'staging-manager']}
            path={`${path}/pausedAssociates`}
            component={PausedAssociates}
          />
          <ProtectedRoute
            allowedRoles={['admin', 'staging-manager']}
            path={`${path}/threeInterviews`}
            component={ThreeInterviews}
          />
          <ProtectedRoute
            allowedRoles={['admin', 'staging-manager']}
            path={`${path}/`}
